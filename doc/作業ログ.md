...
- **改良版 RandomForest v2**
  - 特徴:
    - keyword > 前処理せずダミー変数化
    - location > 記号と数値の削除をしたのち、pycountry による国名への正規化してからダミー変数化
...
- **改良版 RandomForest v3**
  - 特徴:
    - keyword > 前処理せずダミー変数化
    - location > 記号と数値の削除をしたのち、pycountry による国名への正規化してからダミー変数化
    - text > 改行削除等の正規化、リンクを削除、ストップワード除去 TF-IDF でベクトル化
    - 学習時使用したデータは 100%
  - 結果:
    - ローカル (クロス) > 0.62
    - 本番 > 0.77903 提出用 csv には差分が出ているにも関わらずスコアは変化せず？
- **改良版 RandomForest v4**
  - 特徴:
    - keyword > nltk を主としたステミング処理を行いダミー変数化
    - location > 記号と数値の削除をしたのち、pycountry による国名への正規化してからダミー変数化
    - text > 改行削除等の正規化、リンクを削除、ストップワード除去 TF-IDF でベクトル化
    - 学習時使用したデータは 100%
  - 結果:
...
- **改良版 RandomForest v5**
  - 特徴:
    - keyword > nltk を主としたステミング処理を行いダミー変数化、足切り追加
    - location > 記号と数値の削除をしたのち、pycountry による国名への正規化してからダミー変数化、足切り追加
    - text > 改行削除等の正規化、リンクを削除、ストップワード除去 TF-IDF でベクトル化、足切り追加
    - 学習時使用したデータは 100%
  - 結果:
    - ローカル (クロス) > 0.60174607
    - 本番 > 0.76861
- **改良版 RandomForest v5**
  - 特徴:
    ...
    - text > 改行削除等の正規化、リンクを削除、ストップワード除去 TF-IDF でベクトル化、足切り追加
    - 学習時使用したデータは 100%
    - ハイパーパラメータ: RandomForestClassifier のハイパーパラメータをスコアを見ながら調整
  - 結果:
    - ローカル (クロス) > 0.63799636
    - 本番 > 0.73276
- **改良版 RandomForest v6**
...
- **改良版 RandomForest v8**
  - 特徴:
    ...
    - text > 改行削除等の正規化、リンクを削除、ストップワード除去 TF-IDF でベクトル化、足切り追加
    - 学習時使用したデータは 100%
    - ハイパーパラメータ: RandomForestClassifier のハイパーパラメータをスコアを見ながら調整、max_features を log2 に変更
    - 交差検証方法をシャッフル有に変更
    - 全体的にパラメータを見直し
  - 結果:
    - ローカル (クロス) > 0.79351348
