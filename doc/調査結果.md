...
本ドキュメントでは、Kaggle の仕様に関する調査結果をまとめる。
...
Kaggle API か WEB 上から提出可能。一日あたりの提出回数制限がある。WEB 上の Notebook を開いて右の「Submit to Competition」を開くとある「Daily Submissions」に回数が表示される。
ちなみに "NLP Getting Started Tutorial" コンペでは一日あたり 5 回まで提出可能だった。
